...
# (3) log4j-1.2.16.jar
# (4) TableLayout.jar
export CLASSPATH=$CLASSPATH:$PWD/jar/jmetal4.5_debug.jar:$PWD/jar/patus.jar:$PWD/jar/log4j-1.2.16.jar:$PWD/jar/TableLayout.jar
...
for sx in $SIZE 
do 
  input="$sx 1024" # this 1024 is not an error! check Table 4 in the paper! :D
  echo $input
...
echo "optimizing six order laplacian"
...
echo "optimizing horizontal interpolation"
stencilname="hinter.stc"
export STENCIL_HOME=$DSL_BUILD/hinter.stc
SIZE="1024"
for sx in $SIZE 
do 
  input="$sx $sx"  
...
echo "optimizing vertical interpolation"
stencilname="vinter.stc"
export STENCIL_HOME=$DSL_BUILD/vinter.stc
SIZE="1024"
for sx in $SIZE 
do 
  input="$sx $sx"  
