...
def violin_plot(ax, data, pos, bp=False):
	...
	logpos = []
	for p in pos:
		logpos.append(r"$2^{"+str(int(math.log(p, 2)))+"}$")
	...
	plt.xticks(range(1, len(pos)+1), logpos,rotation=70)
...
for i, filename in enumerate(glob.glob("./data/kendall/*.tau")):
	...
	plt.xlabel(size.replace('_','='),fontsize=fs)
	plt.ylabel(r"Kendall's $\tau$",fontsize=fs)
	plt.grid(True)
	#plt.plot(taus)
	#plt.show()
...
plt.savefig('plots/kendall_violin.png') #, bbox_inches='tight')
with PdfPages('plots/kendall_violin.pdf') as pdf:
	pdf.savefig(plt.gcf())
